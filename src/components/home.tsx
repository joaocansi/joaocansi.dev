...
function Home() {
  return (
    <section
      id="home"
      className={clsx(
        'flex min-h-[calc(100vh-theme(height.24))] justify-center'
      )}
    >
      <div
        className={clsx(
          'mb-20 flex w-full items-center justify-center gap-10',
          'max-xl:mt-5 max-lg:flex-col-reverse'
        )}
      >
        <img
          className={clsx('w-72', 'md:w-[450px]')}
          src="/dog-dancing.gif"
          alt="Dog Dancing"
        />
        <div
          className={clsx(
            'flex w-1/2 flex-col gap-5',
            'max-lg:w-full xl:ml-10'
          )}
        >
          <p className={clsx('text-xl font-black uppercase', 'max-xl:text-lg')}>
            Hey,
          </p>
          <p className={clsx('text-base', 'max-xl:text-sm')}>
